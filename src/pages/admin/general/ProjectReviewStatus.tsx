...
const initialData: ReviewStatus[] = [
...
export default function ProjectReviewStatus() {
  const [data, setData] = useState<ReviewStatus[]>(initialData)
  const [isDialogOpen, setIsDialogOpen] = useState(false)
  const [editingItem, setEditingItem] = useState<ReviewStatus | null>(null)
  const [deleteItem, setDeleteItem] = useState<ReviewStatus | null>(null)
  ...
  const handleAdd = () => {
    setEditingItem(null)
    setFormData({ projectName: '', status: 'Pending Review', reviewer: '', submittedDate: '' })
    setIsDialogOpen(true)
  }

  const handleEdit = (item: ReviewStatus) => {
    setEditingItem(item)
    setFormData({
  ...
  const handleSave = () => {
    ...
    if (editingItem) {
      ...
      setData([...data, { id: newId, ...formData, lastUpdated: today }])
    }
  ...
  return (
    <div className="p-8">
              ...
              <tr key={row.id} className="hover:bg-gray-50">
                ...
                <td className="whitespace-nowrap px-6 py-4 text-sm">
                  <button
                    ...
                    className="text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded p-1.5 mr-2"
                  >
                    Edit
                  </button>
                  <button
      ...
      {isDialogOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setIsDialogOpen(false)}>
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg mx-auto" onClick={(e) => e.stopPropagation()}>
      ...
      {deleteItem && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setDeleteItem(null)}>
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md mx-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to delete <strong>{deleteItem.projectName}</strong>? This action cannot be undone.
            </p>
